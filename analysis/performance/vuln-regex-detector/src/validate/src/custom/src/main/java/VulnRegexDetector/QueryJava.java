...
public class QueryJava {
  public static void main(String[] args) throws IOException {
    if (args.length == 1) {
      ...
      log(String.format(
          "matching: pattern /%s/, input: len %d value <%s>", pattern, input.length(), input));
      ...
      try {
        // TODO: Create custom driver code for project
        // If possible use the same public interface as the Java matcher
      ...
      if (validPattern) {
        ...
        if (matched) {
          ...
          for (int i = 1; i <= matcher.groupCount(); i++) {
            ...
            // Strictly speaking, there is a distinction between "did not match" and "matched the
            // empty string".
            if (grp == null) {
              captureGroups.add("");
            } else {
      ...
      MyMatchResult matchResult =
          new MyMatchResult(validPattern, inputLength, matched, matchContents);
...
class MyMatchResult {
  ...
  public MyMatchResult(
      boolean validPattern, int inputLength, boolean matched, MyMatchContents matchContents) {
