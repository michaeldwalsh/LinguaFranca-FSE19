...
def main():
    ...
    try:
        ...
        # Try a match
        log(
            "matching: pattern /{}/ input: length {}".format(
                obj["pattern"], len(obj["input"])
            )
        )
        # matchResult = regexp.match(obj['input']) # Full-match semantics -- better case
        matchResult = regexp.search(
            obj["input"]
        )  # Partial-match semantics -- worse case
        ...
        if matchResult:
            ...
            obj["matchContents"] = {
                ...
                "captureGroups": [
                    g if g is not None else "" for g in matchResult.groups()
                ],
            }
        else:
            obj["matched"] = 0
