...
def getTimedOutSet(slras):
    return set(filter(lambda slra: slra.everTimedOut(), slras))


def getCrossRegistrySet(slras):
    return set(filter(lambda slra: len(slra.regex.registriesUsedIn()) > 1, slras))
...
def makeReport_variantEffectiveness(allSLRAs):
    ...
    ORIG_NOTVULN_VARIANT_VULN = (
        "orig not vuln, some variant vuln"  # < -- This is the exciting on
    )
    ...
    for slra in allSLRAs:
        ...
        for do in slra.detectorOpinions:
            if not do.timedOut and do.canAnalyze:
                if do.isVuln:
                    if (
                        hasattr(do, "patternVariant")
                        and do.patternVariant == slra.regex.pattern
                    ):
        ...
        if (
            slra.predictedPerformanceInLang("JavaScript")
            == libLF.SLRegexAnalysis.PREDICTED_PERFORMANCE["POW"]
            or slra.predictedPerformanceInLang("JavaScript")
            == libLF.SLRegexAnalysis.PREDICTED_PERFORMANCE["EXP"]
        ):
    ...
    print(
        formatStr
        % ("Case", "Num reported vulnerabilities", "Num actual vulnerabilities in JS")
    )
        ...
        if nPredicted != 0:
            print(
                ...
                % (
                    case,
                    str(nPredicted),
                    "{} ({:.0f}%)".format(nVulnInJS, 100 * nVulnInJS / nPredicted),
                )
            )
...
def makeReport_perRegistryTimeouts(allSLRAs, visDir):
    ...
    for registry in sorted(registries):
        ...
        print(
            tableFormat
            % (
                registryToSLTestLanguage(registry),
                len(registry2slras[registry]),
                len(registry_timedOut),
                percTimedOut,
            )
        )
            ...
            "df": pd.DataFrame.from_records(
                data=[
                    (
                        slra.regex.pattern,
                        nick2full(lang),
                        pred2pretty[slra.predictedPerformanceInLang(lang)],
                    )
                    for slra in allSLRAs
                    ...
                    if (
                        lang
                        in [
                            registryToSLTestLanguage(r).lower()
                            for r in slra.regex.registriesUsedIn()
                        ]
                    )  # Restrict to the languages this regex is syntactically valid in
                    and (
                        lang in slra.regex.supportedLangs
                        and slra.predictedPerformanceInLang(lang)
                        != libLF.SLRegexAnalysis.INVALID_PATTERN
                    )
                ],
                columns=[
                    cols["Pattern"],
                    cols["Language"],
                    cols["Predicted performance"],
                ],
            ),
        },
        {
            ...
            "df": pd.DataFrame.from_records(
                data=[
                    ...
                    for lang in slra.lang_pump2timedOut.keys()
                    # Restrict to the languages this regex is syntactically valid in
                    if lang in slra.regex.supportedLangs
                    and slra.predictedPerformanceInLang(lang)
                    != libLF.SLRegexAnalysis.INVALID_PATTERN
                ],
                columns=[
    ...
    for plotInfo in plotInfos:
        ...
        langCounts = (
            df.groupby([cols["Language"], cols["Predicted performance"]])["lang"]
            .count()
        ...
        percs = (
            langCounts.groupby(
                level=0
            )  # Grouped by language, go through each perf count
            .apply(
                lambda x: 100 * (x / float(x.sum()))
            )  # x / sum(xs): proportion for each perf in this lang
        ...
        ax = sns.barplot(
            x=cols["Language"],
            y=cols["Percentage"],
            hue=cols["Predicted performance"],
            # data=percs,
            data=noLin,
...
def heatmap(
    data, row_labels, col_labels, fig=None, ax=None, cbar_kw={}, cbarlabel="", **kwargs
):
    """
    ...
    ax.grid(
        which="minor", color="w", linestyle="-", linewidth=10, axis="y"
    )  # Sep b/t rows
    ax.grid(
        which="minor", color="w", linestyle="-", linewidth=1, axis="x"
    )  # Sep b/t cols
...
def annotate_heatmap(
    ...
    **textkw
    ...
    if threshold is not None:
        ...
        threshold = im.norm(dataPlotted.max()) / 2.0
    ...
    for i in range(dataAnnot.shape[0]):
        for j in range(dataAnnot.shape[1]):
            ...
            text = im.axes.text(
                j, i, valfmt(dataAnnot[i, j], None), fontsize="large", **kw
            )
            texts.append(text)
...
def makeReport_portability(allSLRAs, visDir):
            ...
            for dst in allSLTestLanguages():
                ...
                if 0 < simplePortingScore:
                    ...
                    if predPerf == libLF.SLRegexAnalysis.PREDICTED_PERFORMANCE["POW"]:
                        ...
                        sys.exit(-1)
                        raise ValueError(
                            "How could simplePortingScore from {} to {} be {} if predicted perf in {} is {}?".format(
                                src, dst, simplePortingScore, dst, predPerf
                            )
                        )
    ...
    for i, i_lang in enumerate(PLOT_LANG_ORDER):
        for j, j_lang in enumerate(PLOT_LANG_ORDER):
            ...
            if i == j:
                ...
                #  )
                labels_arr[j][i] = "{:.1f}%".format(
                    100
                    * sum(numWorse[i_lang][j_lang])
                    / numRegexesConsidered[i_lang][j_lang]
                )
    ...
    print("---------------")
    print(
        "{}/{} ({:.2f}%) of regexes were super-linar in some language".format(
            numEverSuperLinear, len(allSLRAs), 100 * numEverSuperLinear / len(allSLRAs)
        )
    )
    print(
        "{}/{} ({:.2f}%) of regexes performed worse between some pair of languages".format(
            numWorseInAnyPair, len(allSLRAs), 100 * numWorseInAnyPair / len(allSLRAs)
        )
    )
    print("---------------")
    ...
    im, cbar = heatmap(
        ...
        ax=ax,
        cmap="gray_r",
        cbarlabel="Number of regexes that got worse",
    )

    ax.set_title(
        "Worst-case regex performance differences", pad=75, fontsize="xx-large"
    )
    ax.set_ylabel("Destination language", labelpad=0, fontsize="xx-large")

    texts = annotate_heatmap(
        im,
        dataAnnot=labels_arr,
        valfmt=lambda x, pos: x,
...
def main(slraFile, visDir):
    ...
    libLF.log(
        "{} timed out and cross registry".format(len(allSLRAs_crossRegistry_timedOut))
    )
...
parser = argparse.ArgumentParser(
    description="Analyze the results of testing a bunch of libLF.Regex's for SL behavior"
)
parser.add_argument(
    "--slra-file",
    type=str,
    help="In: File of libLF.SLRegexAnalysis objects",
    required=True,
    dest="slraFile",
)
parser.add_argument(
    "--vis-dir",
    help="Out: Where to save plots?",
    required=False,
    default="/tmp/vis",
    dest="visDir",
)
