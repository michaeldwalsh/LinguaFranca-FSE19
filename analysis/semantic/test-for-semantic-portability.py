...
# Dependencies
INPUT_GENERATOR = os.path.join(
    os.environ["ECOSYSTEM_REGEXP_PROJECT_ROOT"], "bin", "gen-input-for-regex.py"
)
...
class MyTask(libLF.parallel.ParallelTask):
    ...
    def _queryRegexInLang(self, pattern, queryFile, language):
        ...
        with tempfile.NamedTemporaryFile(
            prefix="SemanticAnalysis-queryRegexInLang-OutFile-",
            suffix=".json",
            ...
            completedProcess = subprocess.run(
                ...
                timeout=MAX_REGEX_QUERY_TIME_SEC,
            )
            ...
            if completedProcess.returncode == 0:
                ...
                for result in queryResult["results"]:
                    ...
                    mc = libLF.MatchContents().initFromRaw(
                        rawMC["matchedString"], rawMC["captureGroups"]
                    )
                    matchResult = libLF.MatchResult().initFromRaw(matched, mc)
                    rer = libLF.RegexEvaluationResult(
                        pattern, result["input"], language, matchResult
                    )
                    rers.append(rer)
                return rers
            else:
    ...
    def _getInputs(self):
        ...
        # Query from tempfile
        with tempfile.NamedTemporaryFile(
            prefix="SemanticAnalysis-genInputs-",
            suffix=".json",
            delete=DELETE_TMP_FILES,
        ) as queryFile, tempfile.NamedTemporaryFile(
            prefix="SemanticAnalysis-outFile-", suffix=".json", delete=DELETE_TMP_FILES
        ) as outFile:
            libLF.writeToFile(queryFile.name, self.regex.toNDJSON())
            rc, out = libLF.runcmd(
                "'{}' --regex-file '{}' --out-file '{}' --parallelism 1 --seed {} --max-inputs-per-generator {} --generator-timeout {} 2>/dev/null".format(
                    INPUT_GENERATOR,
                    queryFile.name,
                    outFile.name,
        ...
        inputs = []
        libLF.log(
            "_getInputs: The {} producers yielded {} total inputs".format(
                len(rpai.stringsByProducer), len(rpai.getUniqueInputs())
            )
        )
        for producer in rpai.stringsByProducer:
            ...
            if 0 < self.maxInputsPerGenerator and self.maxInputsPerGenerator < len(
                ...
                libLF.log(
                    "_getInputs: producer {} yielded {} inputs, reducing to {}".format(
                        producer, len(producerInputs), self.maxInputsPerGenerator
                    )
                )
                producerInputs = random.sample(
                    producerInputs, self.maxInputsPerGenerator
                )
    ...
    def run(self):
        try:
            ...
            libLF.log(
                "  Testing each input in {} langs".format(
                    len(self.regex.supportedLangs)
                )
            )
            ...
            lang2rers = self._evaluateRegex(
                self.regex.pattern, inputs, self.regex.supportedLangs
            )
            ...
            for testString in inputs:
                possibleWitnesses[
                    testString
                ] = libLF.SemanticDifferenceWitness().initFromRaw(
                    self.regex.pattern, testString
                )
            ...
            # Return
            libLF.log(
                "Completed regex /{}/ ({} witnesses out of {} inputs)".format(
                    self.regex.pattern,
                    len(self.regex.semanticDifferenceWitnesses),
                    self.regex.nUniqueInputsTested,
                )
            )
            sys.stderr.flush()
            return self.regex
...
def getTasks(regexFile, maxInputsPerGenerator, rngSeed, generatorTimeout):
    ...
    tasks = [
        MyTask(regex, maxInputsPerGenerator, rngSeed, generatorTimeout)
...
def main(
    regexFile, outFile, parallelism, maxInputsPerGenerator, rngSeed, generatorTimeout
):
    libLF.log(
        "regexFile {} outFile {} parallelism {} maxInputsPerGenerator {} rngSeed {} generatorTimeout {}".format(
            regexFile,
            outFile,
            parallelism,
            maxInputsPerGenerator,
            rngSeed,
            generatorTimeout,
        )
    )
    ...
    with open(outFile, "w") as outStream:
        for regex in results:
            ...
            if type(regex) is libLF.Regex:
                ...
                nExceptions += 1
    libLF.log(
        "Successfully performed cross-language semantic equivalence testing on {} regexes, {} exceptions".format(
            nSuccesses, nExceptions
        )
    )
    ...
    libLF.log(
        "\n  {} ({:.2f}%) of the {} completed regexes had at least one witness for different behavior".format(
            nRegexesWithDifferences,
            100 * (nRegexesWithDifferences / len(completedRegexes)),
            len(completedRegexes),
        )
    )
...
parser = argparse.ArgumentParser(
    description="Test a set of libLF.Regex's for different behavior in different languages"
)
parser.add_argument(
    "--regex-file",
    type=str,
    help="In: File of libLF.Regex objects that have supportedLangs populated",
    required=True,
    dest="regexFile",
)
parser.add_argument(
    "--out-file",
    type=str,
    help="Out: File of libLF.Regex objects with the semanticDifferenceWitnesses field populated",
    required=True,
    dest="outFile",
)
parser.add_argument(
    "--parallelism",
    type=int,
    help="Maximum cores to use",
    required=False,
    default=libLF.parallel.CPUCount.CPU_BOUND,
    dest="parallelism",
)
parser.add_argument(
    "--max-inputs-per-generator",
    type=int,
    help='Maximum inputs to use from each generator (default 100; -1 means "all")',
    required=False,
    default=100,
...
parser.add_argument(
    "--rngSeed",
    type=int,
    help="Seed to use for reproducibility (default -1: random seed)",
    required=False,
    default=-1,
    dest="rngSeed",
)
parser.add_argument(
    "--generator-timeout",
    type=float,
    help="Time out input generators if they takes more than T seconds, and scrape the output for the strings generated so far (default 10, give -1 for no limit)",
    required=False,
    default=10,
...
main(
    ...
    args.outFile,
    args.parallelism,
    args.maxInputsPerGenerator,
    args.rngSeed,
    args.generatorTimeout,
)
