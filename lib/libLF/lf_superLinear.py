...
class PumpPair:
    ...
    def toNDJSON(self):
        _dict = {"prefix": self.prefix, "pump": self.pump}
...
class EvilInput:
    ...
    def toNDJSON(self):
        _dict = {"couldParse": self.couldParse}
        if self.couldParse:
...
class SLRegexDetectorOpinion:
    ...
    def toNDJSON(self):
        ...
        if not self.timedOut:
            if hasattr(self, "patternVariant"):
                _dict["patternVariant"] = self.patternVariant
            if self.isVuln:
                _dict["evilInputs"] = [
                    json.loads(ei.toNDJSON()) for ei in self.evilInputs
                ]
        return json.dumps(_dict)
...
class SLRegexAnalysis:
    ...
    DEFAULT_VULN_REGEX_DETECTOR_ROOT = os.path.join(
        os.environ["ECOSYSTEM_REGEXP_PROJECT_ROOT"],
        "analysis",
        "performance",
        "vuln-regex-detector",
    )
    ...
    PREDICTED_PERFORMANCE = {"LIN": "LIN", "POW": "POW", "EXP": "EXP"}
    ...
    def __init__(
        self,
        regex=None,
        slTimeout=MATCH_TIMEOUT_SEC,
        powerPumps=POW_PUMPS,
        ...
        self.queryDetectorsScript = os.path.join(
            vrdPath, "src", "detect", "detect-vuln.pl"
        )
        self.testInLanguageScript = os.path.join(
            vrdPath, "src", "validate", "validate-vuln.pl"
        )
    ...
    def toNDJSON(self):
        _dict = {
            ...
            "detectorOpinions": [
                json.loads(do.toNDJSON()) for do in self.detectorOpinions
            ],
    ...
    def initFromNDJSON(self, jsonStr):
        ...
        self.detectorOpinions = [
            SLRegexDetectorOpinion().initFromDict(doDict)
            for doDict in obj["detectorOpinions"]
        ]
    ...
    def queryDetectors(self):
        ...
        query = {
            ...
            "memoryLimit": 2048
            * 1024,  # KB each detector gets to use to make a decision. TODO Update VRD docs which say 'in MB'? But cf. detect-vuln.pl:59
        }

        # Query from tempfile
        with tempfile.NamedTemporaryFile(
            prefix="SLRegexAnalysis-queryDetectors-", suffix=".json", delete=True
        ) as ntf:
            libLF.writeToFile(ntf.name, json.dumps(query))
            rc, out = libLF.runcmd(
                "VULN_REGEX_DETECTOR_ROOT={} '{}' '{}' 2>>/tmp/err".format(
                    self.vrdPath, self.queryDetectorsScript, ntf.name
                )
            )
            out = out.strip()
        ...
        for do in self.detectorOpinions:
            if (
                do.isVuln
                and len(list(filter(lambda ei: ei.couldParse, do.evilInputs))) > 0
            ):
                if hasattr(do, "patternVariant") and do.pattern == do.patternVariant:
                    maybeVuln_exact = True
                else:
                    maybeVuln_variant = True
        libLF.log(
            "Maybe vuln: exact {} variant {}".format(maybeVuln_exact, maybeVuln_variant)
        )
    ...
    def _qd_convOutput2DetectorOpinions(self, queryDetectorsOutput):
        try:
            ...
            # TODO Not sure if this can go wrong.
            libLF.log(
                "Could not parse queryDetectorsOutput: <{}> --> {}".format(
                    err, queryDetectorsOutput
                )
            )
            return None
    ...
    def validateDetectorOpinionsInLang(self, lang):
        ...
        libLF.log(
            "Validating detector opinions for <{}> in {}".format(
                self.regex.pattern, lang
            )
        )
                        ...
                        for slrv in slrvs:
                            ...
                            if slrv.validPattern:
                                ...
                                if slrv.nPumps not in self.lang_pump2timedOut[lang]:
                                    ...
                                    self.lang_pump2timedOut[lang][slrv.nPumps] is False
                                ):
        ...
        libLF.log(
            "validateDetectorOpinionsInLang: regex <{}> lang_pump2timedOut[{}] = {}".format(
                self.regex.pattern, lang, self.lang_pump2timedOut[lang]
            )
        )
        return self
    ...
    def _testEvilInputInLang(self, evilInput, lang):
        ...
        for nPumps in [self.EXP_PUMPS, self.powerPumps]:
            ...
            with tempfile.NamedTemporaryFile(
                prefix="SLRegexAnalysis-validateOpinion-", suffix=".json", delete=True
            ) as ntf:
                libLF.writeToFile(ntf.name, json.dumps(query))
                rc, out = libLF.runcmd(
                    "VULN_REGEX_DETECTOR_ROOT={} '{}' '{}' 2>>/tmp/err".format(
                        self.vrdPath, self.testInLanguageScript, ntf.name
                    )
                )
                out = out.strip()
            libLF.log("Got rc {} out\n{}".format(rc, out))
            slRegexVals.append(
                SLRegexValidation(self.regex.pattern, evilInput, json.loads(out))
            )
        ...
        try:
            ...
            for do in self.detectorOpinions:
                ...
                assert (
                    not do.canAnalyze
                    or not do.isVuln
                    or not do.evilInputs[0].couldParse
                )
    ...
    def timedOutInLang(self, lang):
        return (
            self.predictedPerformanceInLang(lang) is self.PREDICTED_PERFORMANCE["POW"]
            or self.predictedPerformanceInLang(lang)
            is self.PREDICTED_PERFORMANCE["EXP"]
        )
    ...
    def predictedPerformancePortingScore(self, sourceLang, destLang):
        ...
        score = (
            measured_perf2val[self.predictedPerformanceInLang(destLang)]
